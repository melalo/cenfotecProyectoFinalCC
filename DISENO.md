...
| Qué pasa con una cita que **ya pasó** | Un caso aparte con su propio mensaje; que caiga en la misma regla | **Que caiga en la misma regla** — *revertido el 2026-08-24 por RN-26: ahora sí es un caso aparte, con su propio motivo `ya_paso`* | La razón de entonces era buena y se quedó corta: «si faltan −22 horas, faltan menos de 4», así que no hacía falta ningún caso aparte y era un caso borde menos donde equivocarse. **Lo que la pieza 8 mostró es que sí hacía falta, y por dos motivos distintos.** El primero ya se había visto el 2026-08-20 y se había tapado con un parche: `porQueNoSePuedeCambiar`, una segunda función que existía **solo** para que la pantalla no dijera la frase falsa «faltan menos de 4 horas» debajo de una cita del mes pasado. El segundo lo trajo RN-26: para Personal la regla de las 4 horas **no corre**, así que una cita pasada no caía en ninguna regla y le quedaba cambiable. Con el caso aparte escrito de verdad —`ya_paso` devuelto por `revisarSiSePuedeCambiar`— las dos cosas se arreglan de una sola vez, y el parche desaparece en vez de crecer. |
...
## Decisiones tomadas al construir la pieza 8

*Construida el 2026-08-24. Es la pieza que cierra las citas pasadas (RF-21) y la que **estrenó una
regla de negocio nueva, RN-26**, escrita en `ESPECIFICACION.md` antes de tocar una línea de código.
No nació ningún archivo nuevo del lado del servidor: todo entró en `reservas.js` y en
`rutas/personal.js`, que ya existían.*

**Las dos decisiones que la estudiante tomó antes de que se escribiera nada** están arriba de todo
porque son las que mandaron el resto:

| Decisión | Opciones consideradas | Elección | Razón |
|---|---|---|---|
| Qué botones ve Personal sobre una cita que ya pasó y nadie cerró *(la pregunta que la pieza 7 dejó abierta a propósito)* | Los **cuatro** (Reagendar, Cancelar, Completada, No asistió); **solo los dos de cerrar** | **Solo los dos de cerrar**, decidido por la estudiante el 2026-08-24 con la aplicación levantada | La razón de fondo es **RN-19**: reagendar **mueve la misma cita** a otra fecha, no crea una segunda, así que mover la cita de alguien que no se presentó **borra del registro que faltó** — y RN-19 existe precisamente para que la cita perdida deje constancia de por qué se perdió. Cancelarla tendría el problema al revés: diría que se canceló algo que en realidad ocurrió. **El caso real que motivaba dejarlos igual se resuelve en dos pasos:** quien faltó llama, la asistente cierra la vieja como «no asistió» y le reserva una cita nueva. Un paso más, y a cambio el historial dice las dos cosas. **La regla se escribió primero** en `ESPECIFICACION.md` como **RN-26**, tal como la pieza 7 había dejado anotado que había que hacer si se decidía restringir. |
| La dirección del endpoint que cierra la cita | `PATCH /api/citas/:citaId/cierre`, como decía el plan; `PATCH /api/personal/citas/:citaId/cierre`, como manda la convención | **Bajo `/api/personal/`**, decidido por la estudiante el 2026-08-24 | Dos documentos de la carpeta decían cosas distintas. El bloque *Produce* de la pieza 8 se escribió **antes** de que la pieza 7 fijara la convención «todo lo que solo abre Personal vive bajo `/api/personal/`», que tiene su razón escrita: **el permiso se lee de un vistazo en la dirección**, sin abrir el código. Y el propio bloque se contradecía —el `GET /api/personal/citas-por-cerrar` sí la seguía—, así que no era una excepción pensada sino un descuido de fecha. Ganó la convención más nueva, y **`PLAN.md` se corrigió con la razón anotada**, que es lo que las reglas del proyecto mandan cuando la construcción revela que un documento quedó viejo. |

**Y las decisiones técnicas que salieron de construirla:**

| Decisión | Opciones consideradas | Elección | Razón |
|---|---|---|---|
| Dónde entra RN-26 en la regla de «¿se puede cambiar esta cita?» | Un `if` aparte en cada endpoint; adentro de `revisarSiSePuedeCambiar` **después** de la excepción de Personal; **antes** de esa excepción | **Adentro de la misma función, antes de la excepción de Personal** | Ahí está toda la regla, en una línea. Puesta **después**, Personal —que no tiene ventana de cancelación (RN-6)— seguiría pudiendo mover y cancelar citas del mes pasado, o sea que la regla no habría hecho nada: es justo el hueco que se venía a tapar. Y en `if` sueltos en los archivos de rutas habría quedado la misma regla escrita dos veces, que es lo que este proyecto no hace. **Las dos reglas no se pisan porque miran cosas distintas:** RN-26 mira si la cita **ya ocurrió**, RN-5 mira **cuántas horas faltan** para una que no ocurrió. |
| Con qué motivo se rechaza una cita pasada | Seguir contestando `ventana_de_cancelacion`; un motivo propio, **`ya_paso`** | **Un motivo propio, `ya_paso`, con `422`, y para los dos actores** | El motivo viejo era **cierto como cuenta y falso como explicación**: «faltan menos de 4 horas» debajo de una cita de la semana pasada. Se sostenía mientras el único actor que llegaba ahí era el cliente; con dos actores dejó de sostenerse. Sigue siendo `422` y no `403` con el criterio de siempre: **el problema no es el permiso sino el momento**. |
| Qué pasa con `porQueNoSePuedeCambiar`, la función que existía solo para traducir ese motivo | Borrarla; dejarla como estaba; **adelgazarla** | **Adelgazarla**: hoy solo convierte `"se_puede"` en `null` | Nació el 2026-08-20 **como un parche**: la regla decía `ventana_de_cancelacion` y la pantalla necesitaba decir otra cosa, así que se agregó una segunda función que traducía **solo para la pantalla**. Con RN-26 el parche dejó de hacer falta, y no porque se borrara sino **porque la regla de verdad cambió**: lo que la pantalla decía y lo que la regla decide pasaron a ser lo mismo. Se dejó la función porque la conversión que queda —`"se_puede"` → `null`— sigue siendo real, y porque su comentario guarda una lección que este proyecto ya vivió dos veces: **cuando un mensaje suena falso, casi siempre la regla detrás tiene un hueco.** Pasó acá y pasó con RN-25. |
| Dónde se contesta «¿esta cita ya pasó?» | En cada lugar que lo necesita; en una función, **`laCitaYaPaso`** | **En una función, apoyada en `todaviaNoEmpezo` de `tiempo.js`** | Tres cosas dependen de esa respuesta: RN-26, la lista de citas por cerrar y en qué grupo cae la cita en pantalla. Con tres cuentas separadas, un día una cita podría estar en el historial **y a la vez** ofrecer «Reagendar». La cuenta en sí no vive en `reservas.js` sino en `tiempo.js`, como manda la convención, y es **la misma** que usa RN-25 para los horarios del calendario: así las dos reglas del proyecto que hablan de «ya empezó» no se pueden desincronizar. *De paso se corrigió `grupoDeLaCita`, que hacía su propia cuenta con un borde un milisegundo distinto.* |
| Marcar como completada una cita que **todavía no ocurrió** | Dejarlo pasar; rechazarlo | **Rechazarlo, con `422` y el motivo `todavia_no_paso`** | Sale directo de **RN-17**, que dice que «completada» se marca *después de que el cliente asistió*: nadie asistió todavía a una cita de la semana que viene. Sin esto se podría marcar como completada una cita futura, y esa etiqueta **se desdiría** el día de la cita — que es exactamente lo que la regla de la etiqueta existe para evitar. Es `422` y no `409` por lo mismo de siempre: el problema es el momento. *No estaba en el plan; se adoptó al construir y queda escrito acá como decisión.* |
| Cómo se filtran las citas pasadas para la lista | En el `WHERE` de la consulta, comparando el texto del `inicio`; **en JavaScript, después de traerlas** | **En JavaScript, con `laCitaYaPaso`** | En SQL funcionaría, porque todos los momentos del proyecto se escriben igual y con el mismo desfase. Pero sería **la misma regla escrita dos veces**, con dos bordes que un día pueden dejar de coincidir. Al volumen del negocio —unas 2.300 citas al año (RN-15)— traerlas y filtrarlas no cuesta nada, y a cambio la respuesta a «¿ya pasó?» sigue estando en un solo lugar. |
| Desde dónde se cierra una cita | Solo desde la lista «Citas por cerrar»; solo desde las citas del cliente; **desde los dos lados** | **Desde los dos**, con la misma función | Son dos momentos de trabajo distintos y los dos existen: **«cerrar lo del día»**, que se hace de corrido sobre la lista de todo el negocio, y **«esta persona llamó»**, que se hace mirando sus citas — y ahí los botones caen justo en el renglón donde antes estaban «Reagendar» y «Cancelar», que es lo que la decisión de arriba dejó vacío. **No es la regla escrita dos veces:** los dos llaman a `cerrarLaCita` y mandan el mismo pedido; lo único que cambia es qué lista se vuelve a pedir después y en qué aviso se contesta. |
| Si se pregunta antes de cerrar | Cerrar de una; **preguntar en el mismo renglón** | **Preguntar**, igual que antes de cancelar | **Cerrar no se deshace:** una cita ya cerrada no admite otro estado, así que un toque por error deja escrito que alguien faltó cuando en realidad fue. Y a diferencia de cancelar, **los dos botones están uno al lado del otro**, así que confundirse es más fácil todavía. Es el mismo criterio que la estudiante fijó el 2026-08-20 para cancelar, y la pregunta se muestra igual: reemplazando a los botones en el mismo renglón, sin ventana emergente. |
| Qué devuelve `PATCH …/cierre` | `204`, sin cuerpo; **`200` con `{id, estado, cerradaEn}`** | **`200` con los tres datos** | Lo fija el plan, que dice `200`. Y los tres datos son los que la pantalla acaba de escribir en la base: devolverlos deja comprobar en las pruebas —sin mirar la base— que lo que se guardó es lo que se pidió. *(Cancelar sí devuelve `204`, y la diferencia es real: ahí no hay ningún dato nuevo que contar.)* |
| En qué orden viene la lista de citas por cerrar | Como las devuelva la base; **de la más vieja a la más nueva** | **De la más vieja a la más nueva** | Arriba queda la que más tiempo lleva esperando, que es la que más urge cerrar y también la que más fácil se olvida. *No estaba en el plan; se adoptó al construir y hay una prueba que lo fija —insertando las citas al revés, para que no pase por casualidad.* |
| Cómo se ve el renglón de «Citas por cerrar» | Una tarjeta por cita; **la misma forma que un renglón de «Mis citas»** | **La misma forma**, con el nombre del cliente agregado arriba | Que las dos listas se lean igual es lo que hace que no haya que aprender una pantalla nueva. La única diferencia es el **nombre del cliente**, y hace falta porque es la única lista donde los renglones son de gente distinta: en «Mis citas» el dueño ya lo dice el título de la sección. La clase nueva `.cita__cliente` **no inventa ningún valor**: el tamaño y el peso son los del servicio de abajo, y lo único que la destaca es el navy, que en ese archivo ya es el color de lo importante. |
| Qué etiqueta llevan las citas cerradas | Un color por estado (verde para completada, rojo para no asistió); **el mismo indigo de siempre** | **El mismo indigo**, como «CANCELADA» | `VISUALS.md` menciona colores de estado pero **no dice cuáles**, y la regla del proyecto es que un valor que no está ahí no se inventa en el `.scss`: se pregunta. Queda anotado como algo que la estudiante puede decidir mirando la pantalla. Lo que sí se resolvió es **la palabra**: en la base el estado se escribe `no_asistio`, sin tilde y con guión bajo como todo lo técnico, y en pantalla se lee **«NO ASISTIÓ»** — la traducción está en un solo lugar del JavaScript. |

**Lo que esta pieza NO tocó, a propósito.** `revisarSiSePuedeCambiar` es la función de **CA-3**, uno
de los tres criterios de aceptación que el curso exige proteger con pruebas que corran en cada push,
así que meterle una regla nueva pedía demostrar que no se llevó nada por delante. Hay dos pruebas
escritas para eso solo: la misma cita que empieza dentro de 2 horas se le **sigue** aceptando a
Personal (`204`) y rechazando al cliente (`422 ventana_de_cancelacion`).

### Las decisiones visuales de la revisión de la pieza 8 (2026-08-24)

*Tomadas por la estudiante con la aplicación levantada y mirando la pantalla. Las dos primeras son de
texto y valen para una lista cada una; las dos últimas cambian **cómo se escribe la medida de la
letra en todo el sitio**, así que son las de más alcance de todo el proyecto desde el sistema visual
mismo.*

| Decisión | Opciones consideradas | Elección | Razón |
|---|---|---|---|
| El nombre del cliente y el servicio en «Citas por cerrar» | Dos renglones, uno debajo del otro; **un solo renglón** | **Un renglón: «Marisol Prueba - Limpieza facial»**, en `#1f2247` | En dos renglones se leían como dos datos sueltos; juntos se leen como una frase, que es lo que son: **qué le tocaba a quién**. Y el renglón deja de ser tan alto, que en una lista larga se nota. **Es un cambio de texto en la vista:** ninguna función, ningún endpoint, ninguna prueba. |
| El paso «Elegí tu terapista» | El nombre arriba y la palabra «Terapista» debajo; **un solo renglón** | **Un renglón: «Terapista: Ana»** | La palabra sola no agregaba nada —el título del paso ya dice «Elegí tu terapista», así que las tres opciones repetían la misma palabra tres veces— y pegada al nombre sí dice algo: **qué es ese nombre**. Lo que cambió es **el texto que se ve**, no el dato: `data-nombre` sigue siendo el nombre pelado, que es lo que `marcarElegido` busca para pintar la opción elegida. Separar el dato de lo que se muestra es justo lo que dejó hacer el cambio sin tocar nada más. |
| El formato de la hora | Hora de 24 (`14:00`); **`am`/`pm`** | **`am`/`pm`, en los seis lugares que escriben una hora** | Cerró un pendiente que `DISENO.md` arrastraba desde el 2026-08-21 — ver la fila tachada en «Pendientes del sistema visual», con el detalle completo. |
| La unidad de los tamaños de letra | Seguir en píxeles; **pasar todo a `rem`** | **`rem`, con `1rem = 16px`**: 41 tamaños de letra y sus 41 interlineados | **No cambió ni un píxel en una pantalla normal**, y ese era el requisito que puso la estudiante. Lo que se gana aparece cuando **la persona que usa la aplicación agranda la letra desde su navegador**, cosa que hace mucha gente que ve poco: con píxeles la aplicación la ignora; con `rem` todo crece junto y proporcionado, porque los tres cortes de pantalla ya estaban en `rem` desde antes. **Lo que NO se convirtió, a propósito:** los espaciados —la retícula de 4px de `VISUALS.md`—, porque son una decisión de *layout* y no de letra, y mezclarlas haría que agrandar la letra desarmara la grilla en vez de solo agrandar el texto; y **el correo**, que sigue en píxeles porque los programas de correo no son navegadores y muchos no entienden `rem` (la misma razón por la que ese archivo usa `<table>`). **Y `html` quedó a propósito sin `font-size`:** ponerle `16px` dejaría el `rem` clavado y toda la conversión no serviría de nada. |
| **El tamaño de toda la letra de la aplicación** | Dejarla como estaba; achicarla reescribiendo los 82 valores; **achicarla con una sola línea** | **Un 20% más chica, con `html { font-size: 80% }`** | Lo pidió la estudiante el 2026-08-24 mirando la pantalla, buscando que la aplicación entrara más holgada. **Se hizo con un porcentaje y no con un tamaño fijo** porque un porcentaje se mide contra la letra base del navegador de quien mira: quien la agranda desde su configuración sigue viendo todo crecer, solo que partiendo de un 20% menos. Escrito `12.8px` quedaría clavado y toda la conversión a `rem` no serviría de nada. **Y en una línea y no en los 82 valores** porque el resultado en pantalla es idéntico y a cambio los `rem` del archivo siguen diciendo de un vistazo qué es cada cosa —`1rem` el texto normal, `1.5rem` un título— en vez de volverse una lista de números sin relación. Volver atrás es borrar esa línea. **Dos efectos que hay que tener presentes:** los espaciados, rellenos y tamaños mínimos para tocar siguen en píxeles, así que las cajas quedan igual y el texto adentro más chico —para las fichas de horario eso es justo lo que se buscaba—; y los tres cortes de pantalla **no se mueven**, porque adentro de un `@media` los `rem` se miden siempre contra la letra base del navegador y nunca contra esta línea. ⚠️ **El costo, escrito y no escondido:** `VISUALS.md` dice *«Body: 16px remains the standard for accessibility»*, y con el 80% el texto normal queda en 12.8px y los avisos y textos de ayuda en 9.6px, por debajo de ese mínimo. **La estudiante lo decidió después de que se le advirtiera**, y por eso quedó anotado en los tres lados: el `.scss`, `VISUALS.md` y acá. |
| El tamaño de los títulos | Fijo; un `@media` por corte de pantalla; **`clamp()`** | **`clamp()` en los seis títulos**, con tres escalas guardadas en variables | `clamp(mínimo, ideal, máximo)` deja que el título **crezca con el ancho de la pantalla** en vez de saltar de golpe al cruzar un corte. **Los tres máximos son los tamaños que la aplicación ya tenía** —32px el del encabezado, 24px los de sección, 18px el del cartel de reagendar—, así que en pantalla grande no cambió nada; lo que se agrega es que en pantalla angosta bajan un escalón en vez de quedarse grandes y apretar todo lo demás. **Reemplazó un `@media` que ya existía:** el `<h1>` pasaba de 28px a 32px de un salto al llegar a la tableta. **Y el interlineado de esos seis pasó a ser un número sin unidad** —`1.333`, o sea «una vez y un tercio»—, que es la parte que se pasa por alto: con un interlineado fijo en `rem`, un título que se achica a 20px se quedaría con el aire de uno de 24 y flotaría. Cada proporción es exactamente la que ese título ya tenía en su tamaño máximo. **La parte `vw` va siempre sumada a un `rem` y nunca sola:** con `vw` solo, quien agranda la letra de su navegador no vería ningún cambio, porque `vw` mide la ventana y no la letra. |

**Lo que estas dos últimas NO arreglaron, y por qué hizo falta una tercera decisión.** Pasar a `rem`
con la letra base en 16px **no cambia ni un píxel** de cómo se ve hoy: es exactamente el mismo dibujo.
Así que el desborde de la ficha de horario con `10:00am` **seguía tal cual** — la conversión preparaba
el terreno, no resolvía ese caso. Se midió: la ficha del calendario es la más apretada del proyecto
—**ocho horarios por día en una fila de cuatro columnas**— y en una pantalla de 320px le quedan unos
58px de texto para un `10:00am` que mide unos 66. **La estudiante lo resolvió ese mismo día dejando
las fichas en hora de 24**, que es la única excepción al formato de todo el proyecto y está escrita
con su razón en «Pendientes del sistema visual».

**Y una lección de método que quedó del día**, porque se repitió dos veces seguidas: *`rem` y `clamp`
hacen que la letra se adapte a quien mira, no que el texto entre en su caja.* Son dos problemas
...
| ~~**El formato de la hora: 24 horas o am/pm**~~ | **Resuelto el 2026-08-24** | **Gana `am`/`pm`, en toda la aplicación**, elegido por la estudiante mirando la pantalla al revisar la pieza 8. Hasta ese día había **dos formatos a la vez**, y eso era un pendiente y no una decisión: todo mostraba la hora de 24 (`14:00`) menos el cartel de reagendar, que estrenó `am`/`pm` el 2026-08-21 porque la estudiante lo pidió ahí. Ahora dicen `am`/`pm` los cinco lugares donde la hora **se lee sola**: la tarjeta de confirmar, el «cuándo es hoy» del cartel de reagendar, las dos listas de citas y **el correo**. **Con una excepción, decidida el mismo día y por escrito: las fichas de horario del calendario**, que siguen en hora de 24 — ver la fila de abajo. La cuenta está escrita **dos veces a propósito** —`horaConAmPm` en `publico/aplicacion-cliente.js` y `escribirHoraDelMomento` en `servidor/tiempo.js`— por la razón de siempre: el navegador no puede leer nada de `servidor/`, y esto **no es una regla de negocio** sino una forma de escribir; el momento que viaja sigue siendo `2026-08-27T14:00:00-06:00`. *El riesgo que esto traía —`10:00am` es más ancho que `10:00`— se midió y se resolvió el mismo día, en la fila de abajo.* |

| ~~**Las fichas de horario del calendario, con `am`/`pm` o sin**~~ | **Resuelto el 2026-08-24** | **Sin `am`/`pm`: siguen en hora de 24** (`10:00`, `13:00`), y son **la única hora de toda la aplicación** que no lo lleva. Elegido por la estudiante mirando la pantalla, el mismo día que eligió `am`/`pm` para todo lo demás. **La razón se puede medir:** son **ocho horarios por día en una fila de cuatro columnas** —la caja más angosta del proyecto— y en una pantalla de 320px a cada ficha le quedan unos **58px** de texto, mientras que `10:00am` mide unos **66px**: se desborda. `10:00` mide unos 48 y entra con lugar de sobra. **Y acá el formato importa menos**, que es lo que hace que la excepción no cueste nada: son ocho números del mismo día, uno al lado del otro, que se leen como una escala para comparar y elegir —9, 10, 11, 13, 14…— y no como un dato suelto que alguien va a anotar. El `am`/`pm` sí importa donde la hora **se lee sola**: la tarjeta de confirmar, la lista de citas y el correo, que son los tres lugares donde una persona se entera de a qué hora tiene que estar en algún lado. **Lo que hay que mirar en pantalla y queda anotado:** en los horarios de la tarde la ficha dice `13:00` y la tarjeta de confirmar, debajo, `1:00pm` — la misma hora escrita de dos formas en la misma pantalla. Se aceptó a propósito. Las otras dos salidas, si algún día molesta, son escribir `10am`/`1pm` sin los minutos cuando son cero, o bajar a tres columnas en la pantalla más angosta. |
| **El color de las etiquetas de estado** | Cuando la estudiante lo decida mirando la pantalla | Las tres etiquetas —CANCELADA, COMPLETADA y NO ASISTIÓ— salen hoy con **el mismo indigo**. `VISUALS.md` dice que los estados usan «colores estandarizados (por ejemplo verde de éxito, ámbar de alerta)» pero **no dice cuáles son**, y la regla del proyecto es que un valor que no está ahí no se inventa en el `.scss`. Si se decide darles color, es un cambio de una sola clase: `.etiqueta-estado` con un modificador por estado. *Anotado el 2026-08-24, al construir la pieza 8, que es la que trajo las otras dos etiquetas.* |
...
| 🆕 **Toda la letra se dibuja un 20% más chica de lo que dice `VISUALS.md`** | **Planteada el 2026-09-07 por la estudiante, y aplazada por ella hasta después de la entrega.** Lo vio en el inspector del navegador: `.cita__fecha` está escrita `0.875rem` —los 14px de `VISUALS.md` divididos entre 16— y **se dibuja a 11.2px**. No es esa clase: son **las 46** medidas de letra del `.scss`, porque `html { font-size: 80% }` hace que `1rem` valga **12.8px y no 16**. La tabla completa, ya medida: 12px→**9.6**, 14px→**11.2**, 16px→**12.8**, 18px→**14.4**, 24px→**19.2**. El texto normal queda en 12.8px, **por debajo de los 16px que `VISUALS.md` llama mínimo accesible** — y eso ya estaba escrito en `CLAUDE.md` como costo asumido del 2026-08-24, sólo que nadie había puesto los números al lado. **El arreglo es borrar UN renglón** (`html { font-size: 80% }`): con eso las 46 rinden lo que su tabla promete, sin tocar ninguna otra línea. ⚠️ **Lo que hay que revisar si se hace:** todo crece 25%, y hay cajas ajustadas a mano al 80% — sobre todo **las fichas de horario del calendario**, que muestran «10:00» sin `am`/`pm` justamente porque a 320px de ancho no cabe más (la razón está medida en `fichaDeHorario`). Se aplazó porque cambia cómo se ve la aplicación entera y la presentación era al día siguiente: un desacomodo se habría descubierto con el proyecto en pantalla delante del docente. **Descartada de paso una tercera opción**: arreglar sólo la clase que se vio, poniéndole `1.09375rem`. Rendía los 14px, y dejaba **dos reglas de conversión en el mismo archivo** —dividir entre 16 para 45 clases y entre 12.8 para una—, así que el cambio siguiente habría vuelto a meter un valor al 80% sin que nadie lo notara. |
