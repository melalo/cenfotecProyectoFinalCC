...
## Cómo se trabaja acá — reglas del curso

> Estas reglas venían del `CLAUDE.md` de la raíz del repositorio del curso
> (`melalo/laboratorioClaudeCenfotec`), donde este proyecto vivía dentro de la carpeta
> `proyectoFinal/`. Al migrar el proyecto a su propio repositorio (2026-08-26) ese archivo dejó de
> estar en el camino, así que las reglas que siguen aplicando quedaron copiadas acá. Se dejaron
> afuera dos que ya no tienen sentido —«la carpeta del día» y «nada fuera de la carpeta del día»—
> porque en este repositorio hay un solo proyecto y no hay carpetas hermanas que proteger.

### Quién trabaja acá

Soy estudiante y estoy aprendiendo a usar Claude Code para desarrollar software. **Tengo poco
conocimiento de programación.** El objetivo no es solo que el trabajo quede hecho: es que yo
entienda lo que se hizo y por qué.

**La responsabilidad de la entrega, y su defensa frente al docente, es mía.** Puedo usar Claude
...
- **Explicá todo con detalle. Nunca asumas que entiendo**, sobre todo cuando se trata de
  programación, lenguajes, herramientas o cualquier concepto técnico. Explicame en lenguaje simple
  y de manera puntual.
...
- **Cuando necesites un dato, buscalo primero en los archivos del repositorio.** `PROYECTO.md`,
  `ESPECIFICACION.md`, `DISENO.md`, `PLAN.md`, `BITACORA.md`, este `CLAUDE.md` y el historial de
  Git tienen casi todo. La respuesta correcta casi siempre ya está escrita en algún lado.
- **Si después de buscar no lo encontrás, preguntame — antes de decidir y antes de cambiar nada.**
  No inventes el dato, no elijas "lo más razonable" en silencio, no lo dejes para descubrirlo más
  adelante.
...
- Cuando me preguntes cosas, vamos una a la vez, una por una.

### Reglas del curso

- **Todo se construye con Claude Code.** Es el objetivo del curso, no un atajo.
- **No se usan datos reales confidenciales**: ni de personas, ni de negocios, ni credenciales de
  servicios reales. Los datos de prueba son inventados.
- **El encargo original no se toca.** `PROYECTO.md` es mi enunciado del proyecto, escrito por mí, y
  queda tal como lo escribí: es el pedido original y sirve para comparar contra lo que terminó
  saliendo. Las correcciones van en los documentos que derivan de él, nunca en él.

### Propagar los cambios hacia atrás

Cuando el trabajo revela que algo escrito antes quedó incompleto o equivocado, el documento
anterior se corrige primero, y después el trabajo nuevo. Un cambio nunca se aplica solo donde
apareció.

Antes de dar por cerrado cualquier cambio:

1. Buscar qué otros archivos hablan de lo mismo.
2. Listarlos y decir en qué quedaron desactualizados.
3. Actualizarlos, y decir explícitamente cuáles se actualizaron.

### Commits

- El mensaje **nombra el entregable**, no solo la fecha: quien revisa busca en el historial "el
  plan", "la especificación", "la pieza N". Un mensaje genérico esconde el trabajo.
